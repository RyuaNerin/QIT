	...
	public partial class Stasisfield : Form
	{
		...
		public bool GenerateStasisfield( )
		{
			try
			{
				...
				var newSize = new Size( ( int ) System.Windows.SystemParameters.VirtualScreenWidth, ( int ) System.Windows.SystemParameters.VirtualScreenHeight );
				this.Size = newSize;

				// 그래픽스 객체 초기화
				graphicsContext = BufferedGraphicsManager.Current;
				graphicsContext.MaximumBuffer = newSize;
				graphics = graphicsContext.Allocate( this.CreateGraphics( ), new Rectangle( Point.Empty, newSize ) );

				var newPos = new Point( ( int ) System.Windows.SystemParameters.VirtualScreenLeft, ( int ) System.Windows.SystemParameters.VirtualScreenTop );

				// 이미지 캡쳐
				stasisImage = new Bitmap( newSize.Width, newSize.Height );
				using ( Graphics g = Graphics.FromImage( stasisImage ) )
				{
					g.CopyFromScreen( newPos, Point.Empty, newSize );
				}
